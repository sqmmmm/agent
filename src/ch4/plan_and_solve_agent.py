...
# 我们将创建一个主类 PlanAndSolveAgent，它的职责非常清晰：接收一个 LLM 客户端，
# 初始化内部的规划器和执行器，并提供一个简单的 run 方法来启动整个流程。
...
import ast    # ast是python内置的库，用于安全的执行字符串，将其转换为Python表达式
